...
public class PoubelleTest {
	...
	public void setUp() throws Exception {
		...
		try {
			...
			le1 = new Levee(d1, 5.0, pb.getIdPoubelle(), "camion1", "chauffeur1");
			le2 = new Levee(d2, 10.0, pb.getIdPoubelle(), "camion1", "chauffeur1");
			le3 = new Levee(d3, 12.0, pb.getIdPoubelle(), "camion1", "chauffeur1");
	...
	public void testGetLesLevees1()
	{
		...
		pb.addLevee(le2);
		pb.addLevee(le3);
		//System.out.println(pb.getLesLevees().size());
		int an = 2015;
		int mois = 5;
		
		System.out.println(pb.getLesLevees(an, mois).size());
		assertTrue(pb.getLesLevees(an, mois).size() == 2);
	}
	@Test
	public void testGetCout()
	{
		...
		int an = 2015;
		int mois = 5;
		//System.out.println(pb.getCout(an, mois));
		assertTrue(pb.getCout(an, mois) == 1.5);
	}
